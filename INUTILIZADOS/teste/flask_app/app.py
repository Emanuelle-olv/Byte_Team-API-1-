...
def perfil_vereador(id_vereador):
    ...
    vereador = dados.get(
        str(id_vereador), None
    )  # Busca pelo ID do vereador no dicionário
    ...
    if vereador:
        ...
        return (
            ...
            404,
        )  # Retorna um erro 404 se o vereador não for encontrado
...
def todos_vereadores():
    ...
    dados = ler_dados_json(
        "vereadores.json"
    )  # Lê os dados do arquivo 'vereadores.json'
...
def grafico_partido():
    ...
    partido_counts = df[
        "partido"
    ].value_counts()  # Conta a quantidade de vereadores por partido
